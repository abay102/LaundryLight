...
void Demo::BuildTexturedCube()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("mesincuci.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	float vertices[] = {
		// format position, tex coords
		// front
		-0.5, -0.5, 0.5, 0.23, 0.17,0.0f,  0.0f,  1.0f,  // 0
		 0.5, -0.5, 0.5, 0.77, 0.17, 0.0f,  0.0f,  1.0f, // 1
		 0.5,  0.5, 0.5, 0.77, 0.8,  0.0f,  0.0f,  1.0f,// 2
		-0.5,  0.5, 0.5, 0.23, 0.8 ,0.0f,  0.0f,  1.0f, // 3

		// right
		0.5,  0.5,  0.5, 0.33, 0.33,1.0f,  0.0f,  0.0f,  // 4
		0.5,  0.5, -0.5, 0.3, 0.33, 1.0f,  0.0f,  0.0f, // 5
		0.5, -0.5, -0.5, 0.3, 0.3,  1.0f,  0.0f,  0.0f,// 6
		0.5, -0.5,  0.5, 0.33, 0.3, 1.0f,  0.0f,  0.0f, // 7

		// back
		-0.5,  -0.5, -0.5, 0.33, 0.33,0.0f,  0.0f,  -1.0f, // 8 
		 0.5,  -0.5, -0.5, 0.3, 0.33, 0.0f,  0.0f,  -1.0f,// 9
		 0.5,   0.5, -0.5, 0.3, 0.3,  0.0f,  0.0f,  -1.0f,// 10
		-0.5,   0.5, -0.5, 0.33, 0.3, 0.0f,  0.0f,  -1.0f,// 11

		// left
		-0.5, -0.5, -0.5, 0.33, 0.33,-1.0f,  0.0f,  0.0f, // 12
		-0.5, -0.5,  0.5, 0.3, 0.33, -1.0f,  0.0f,  0.0f,// 13
		-0.5,  0.5,  0.5, 0.3, 0.3,  -1.0f,  0.0f,  0.0f,// 14
		-0.5,  0.5, -0.5, 0.33, 0.3, -1.0f,  0.0f,  0.0f,// 15

		// upper
		 0.5, 0.5,  0.5, 0.33, 0.33,0.0f,  1.0f,  0.0f,  // 16
		-0.5, 0.5,  0.5, 0.3, 0.33,0.0f,  1.0f,  0.0f, // 17
		-0.5, 0.5, -0.5, 0.3, 0.3, 0.0f,  1.0f,  0.0f,// 18
		 0.5, 0.5, -0.5, 0.33, 0.3, 0.0f,  1.0f,  0.0f, // 19

		// bottom
		-0.5, -0.5, -0.5, 0.33, 0.33,0.0f,  -1.0f,  0.0f, // 20
		 0.5, -0.5, -0.5, 0.3, 0.33,  0.0f,  -1.0f,  0.0f,// 21
		 0.5, -0.5,  0.5, 0.3, 0.3,   0.0f,  -1.0f,  0.0f,// 22
		-0.5, -0.5,  0.5, 0.33, 0.3, 0.0f,  -1.0f,  0.0f,// 23
	};
...
void Demo::BuildTexturedPlane()
{
	...
	int width, height;
	unsigned char* image = SOIL_load_image("granite_diffusemap.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("granite_specularmap.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	GLfloat vertices[] = {
		...
		-50.0, -0.5, -50.0,  0,  0, 0.0f,  1.0f,  0.0f,
		 50.0, -0.5, -50.0, 100,  0, 0.0f,  1.0f,  0.0f,
		 50.0, -0.5,  50.0, 100, 100, 0.0f,  1.0f,  0.0f,
		-50.0, -0.5,  50.0,  0, 100, 0.0f,  1.0f,  0.0f,
